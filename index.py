...
# Conditional import for faiss to avoid errors when FAISS is not installed
try:
    import faiss
...
# Lazy FAISS type definition to handle missing faiss or GPU support
...
if FAISS_AVAILABLE:
    try:
        ...
        FAISSGPUIndex = Any
        GPUIndexConfig = Any
        FAISSIndex = Any if faiss is None else faiss.Index
else:
    FAISSGPUIndex = Any
    FAISSIndex = Any
    GPUIndexConfig = Any
...
class DistributedFAISSIndex(DistributedIndex):
    def __init__(self, index_type: str, code_size: Optional[int] = None):
        if not FAISS_AVAILABLE:
            raise RuntimeError(
                ...
                "Or use GTR retriever instead of Contriever for log datasets."
            )
        if not FAISS_GPU_AVAILABLE and "gpu" in index_type.lower():
             raise RuntimeError(
                "FAISS GPU support is not available. Please install faiss-gpu if you want to use GPU indexes.\n"
                "Current FAISS installation only supports CPU."
            )
        super().__init__()
        self.embeddings = None
        self.doc_map = dict()
    ...
    def _add_embeddings_by_chunks(self) -> None:
        ...
        for embeddings_chunk in split_embeddings:
            if FAISS_GPU_AVAILABLE and isinstance(self.faiss_gpu_index, FAISSGPUIndex.__args__):
    ...
    def _save_faiss_index(self, path: str) -> None:
        ...
        if FAISS_GPU_AVAILABLE:
            ...
            cpu_index = self.faiss_gpu_index
    ...
    @torch.no_grad()
    def _set_index_config_options(self, index_config: Any) -> Any:
        """
    ...
    def gpu_index_factory(self, dimension: int, n_list: Optional[int] = None) -> FAISSIndex:
        ...
        if not FAISS_GPU_AVAILABLE:
            ...
            return faiss.index_factory(dimension, "Flat", faiss.METRIC_INNER_PRODUCT)
    ...
    def _move_index_to_gpu(self, cpu_index: FAISSIndex) -> None:
        ...
        """
        if not FAISS_GPU_AVAILABLE:
            self.faiss_gpu_index = cpu_index
            return
        self.gpu_resources = faiss.StandardGpuResources()
        cfg = self._set_gpu_options()
