...
def main() -> None:
    ap = argparse.ArgumentParser(description="Gate-mode ablation runner (full / vr_only / no_gate).")
    ...
    ap.add_argument(
        ...
        help="Which ToT-style AIOps mode to ablate (must invoke thought parsing).",
    )
    ap.add_argument(
        "--gate_modes",
        type=str,
        default="full,vr_only,no_gate",
        help="Comma-separated gate modes to run (default: full,vr_only,no_gate).",
    )

    # Keep these aligned with scripts/run_hdfs_aiops_chimera_cder_driver.py::make_base_args expectations.
    ...
    ap.add_argument("--aiops_thought_max_gen_len", type=int, default=512,
                    help="Must be >= 512 for Chinese 4-step thought to complete without truncation.")
    ...
    ap.add_argument("--aiops_skip_thought", type=int, default=0, help="0=run thought loop (default); 1=skip thought.")
    ...
    base_args.aiops_skip_thought = int(args_cli.aiops_skip_thought)
    ...
    gate_modes = [m.strip() for m in str(args_cli.gate_modes or "").split(",") if m.strip()]
    if not gate_modes:
        raise RuntimeError("--gate_modes must be non-empty")

    rows: List[Tuple[str, str]] = []
    all_done = True
    for gm in gate_modes:
        gm = gm.strip().lower()
        if gm not in {"full", "vr_only", "no_gate"}:
            raise RuntimeError(f"Unsupported gate mode: {gm}")
        base_args.gate_mode = gm

        label = f"{dataset_tag} {base_args.retrieval_mode} gate={gm}"
        out_path = str(out_dir / f"{dataset_tag}_gate_mode_{kind}_{base_args.retrieval_mode}_{gm}_{n}_{stamp}.json")
        reason_path = str(out_dir / f"{dataset_tag}_gate_mode_{kind}_{base_args.retrieval_mode}_{gm}_{n}_{stamp}_reasoning.json")
    ...
    metrics_md = str(out_dir / f"{dataset_tag}_gate_mode_{kind}_{base_args.retrieval_mode}_{n}_{stamp}.metrics.md")
    if all_done:
